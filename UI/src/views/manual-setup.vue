...
<template>
	<v-container fluid>
		...
		<v-container fluid justify-center v-if="showCourses">
			...
			<v-container class="text-center">
				...
				<v-row v-for="(course, idx) in store.courses" :key="idx+'-'+course.code">
					...
					<v-col cols='2' class="text-right">
						<v-btn icon="mdi-delete" density="compact" variant="plain" color="#D94D36" @click="deleteCourse(course)" class='pb-1'>
						</v-btn>
					</v-col>
				</v-row>
			</v-container>
			...
			<v-row class="mt-1">
				<v-col class="text-right" >
					<v-btn @click="UploadAgain" class='action_buttons' style='background: #C31F1F; color: #F8F0E3'>
						Upload Again
					</v-btn>
					<v-btn @click="addCourse" class='action_buttons' style='background: #151C55; color: #F8F0E3'>
						Add courses
					</v-btn>
					<v-btn @click="onConfirmation" class='action_buttons' style='background-color: #106B13; color: #F8F0E3'>
						Confirm
					</v-btn>
				</v-col>
			</v-row>
		...
		<v-container text-center fluid v-else>

			<v-card max-width="550" class="mx-auto">
				<v-toolbar>
					<v-toolbar-title class="font-weight-light">Manual Setup</v-toolbar-title>
					<v-spacer></v-spacer>
				</v-toolbar>
				<v-card-text>
					<v-autocomplete
			...
			<v-row class="mt-1 justify-center">
				<v-btn @click="confirmAddCourse" class='action_buttons mt-2' style='background-color: RGB(16, 107, 19, 0.9)'>
				Confirm
				</v-btn>
